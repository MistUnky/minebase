...
function doors.register(name, def)
	...
	minetest.register_lbm({
		...
		action = function(pos, node)
			...
			-- properly place doors_api:hidden at the right spot
			local p3 = p2
			if new.state >= 2 then
				p3 = (p3 + 3) % 4
			end
			...
			minetest.set_node({x = pos.x, y = pos.y + 1, z = pos.z},
				{name = "doors_api:hidden", param2 = p3})
		end
	})
	...
	if not def.sound_open then
		def.sound_open = "doors_api_wood_open"
	end

	if not def.sound_close then
		def.sound_close = "doors_api_wood_close"
	end
...
function doors.register_fencegate(name, def)
	...
	fence_closed.mesh = "doors_api_fencegate_closed.obj"
	fence_closed.gate = name .. "_open"
	fence_closed.sound = "doors_api_fencegate_open"
	...
	fence_open.mesh = "doors_api_fencegate_open.obj"
	fence_open.gate = name .. "_closed"
	fence_open.sound = "doors_api_fencegate_close"
