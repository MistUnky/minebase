...
local function is_owner(pos, name)
	...
	return owner == name or minetest.check_player_privs(name, "protection_bypass")
end

local share_bones_time = tonumber(minetest.settings:get("share_bones_time")) 
	or 1200
local share_bones_time_early = tonumber(minetest.settings
	:get("share_bones_time_early")) or share_bones_time / 4

containers.register_container("bones:bones", {
	closed = {
		description = S("Bones"),
		tiles = {
		...
		on_punch = function(pos, node, player)
			if not is_owner(pos, "dead " .. player:get_player_name()) then
				return
			end
		...
		on_timer = function(pos, elapsed)
			...
			if time >= share_bones_time then
				meta:set_string("infotext", S("@1's old bones", 
...
local function move_items(pos, inv, player_inv)
...
local function bones_time(pos, meta, player_name)
	meta:set_string("formspec", containers.create_formspec("context"))
	meta:set_string("owner", "dead " .. player_name)
...
minetest.register_on_dieplayer(function(player)
	...
	if bones_mode ~= "bones" and bones_mode ~= "drop" and bones_mode ~= "keep" 
		then
		bones_mode = "bones"
	end

	local bones_position_message = minetest.settings
		:get_bool("bones_position_message") == true
	...
	move_items(pos, inv, player_inv)
	bones_time(pos, meta, player_name)
end)
