...
local function tnt_explode(pos, radius, ignore_protection, ignore_on_blast, owner, explode_center)
	...
	basic_flame_on_construct = minetest.registered_nodes["base_fire:flame"].on_construct

	local c_fire = minetest.get_content_id("base_fire:flame")
...
minetest.register_node("tnt:gunpowder", {
	...
	on_punch = function(pos, node, puncher)
		if puncher:get_wielded_item():get_name() == "torches:common" then
...
if enable_tnt then
	...
	minetest.register_abm({
		...
		neighbors = {"base_fire:flame", "base_liquids:lava_source", "base_liquids:lava_flowing"},
		interval = 4,
		chance = 1,
