...
local S = minetest.get_translator("base_cart")
...
local cart_entity = {
	initial_properties = {
		...
		visual = "mesh",
		mesh = "base_cart_cart.b3d",
		visual_size = {x=1, y=1},
		textures = {"base_cart_cart.png"},
	},
...
function cart_entity:on_punch(puncher, time_from_last_punch, tool_capabilities, direction)
	...
	if puncher:get_player_control().sneak then
		...
		if not (creative and creative.is_enabled_for
				...
				or not inv:contains_item("main", "base_cart:cart") then
			local leftover = inv:add_item("main", "base_cart:cart")
...
minetest.register_entity("base_cart:cart", cart_entity)

minetest.register_craftitem("base_cart:cart", {
	description = S("Cart") .. "\n" .. S("(Sneak+Click to pick up)"),
	inventory_image = minetest.inventorycube("base_cart_cart_top.png", "base_cart_cart_front.png", "base_cart_cart_side.png"),
	wield_image = "base_cart_cart_front.png",
	on_place = function(itemstack, placer, pointed_thing)
		...
		if carts:is_rail(pointed_thing.under) then
			minetest.add_entity(pointed_thing.under, "base_cart:cart")
		elseif carts:is_rail(pointed_thing.above) then
			minetest.add_entity(pointed_thing.above, "base_cart:cart")
		else
			return
		end
